...
class GipLocalDB extends ReadyResource {
  ...
  namespace (name) { return this._store.namespace(name) }
  ...
  async _joinCore(key, opts = {}) {
    ...
    // NOTE: do NOT call discovery.refresh() on cache hit. The discovery
    // session already announces/looks-up on its own schedule, and a refresh
    // forces a fresh DHT round-trip + reconnect attempts every time. Hot
    // routes like the home page call getCore() for every repo on every
    // request, so refreshing here churns connections and makes peer counts
    // visibly flicker. The 2nd-sync hang it was meant to mitigate is
    // already covered by _waitForTopicPeer scanning existing connected peers.
  ...
  async openRemote(link, { blind, blindPeerKeys } = {}) {
    if (!blind) {
      blind = blindPeerKeys?.length
        ? new BlindPeering(this._swarm.dht, this._store, { keys: blindPeerKeys })
        : this._blind
    }
    ...
    const remote = new Remote(this._store, { name, key }, { blind })
    await remote.ready()
