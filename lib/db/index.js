...
class GipLocalDB extends ReadyResource {
  ...
  constructor(opts = {}) {
    ...
    this._store =
      opts.store || new Corestore(join(homedir(), '.gip'), { readOnly: !!opts.readonly })
  ...
  async _createRemote(link) {
    ...
    await remote.ready()
    const discovery = this._swarm.join(remote.discoveryKey, {
      server: remote.core.writable,
      client: !remote.core.writable
    })
    ...
    if (this._blind) this._blind.addCoreBackground(remote.core, remote.core.key)
  ...
  async addRemote(link) {
    ...
    if (!existing) {
      await this._db.insert('@gip/repos', { name, key: remote.key })
      await this._db.flush()
    }

    this._remotes.set(name, remote)
    return { remote, isNew: !existing }
  }
  ...
  async openRemote(link) {
    ...
    // Only non-writable cores need to sync from peers before reading
    if (!remote.core.writable) {
