...
class Gip extends ReadyResource {
  ...
  async fetch() {
    ...
    // Collect all objects and refs first, then write once to avoid
    // git's "initial ref transaction called with existing refs" error during clone
    const allObjects = []
    const allRefs = {}
    const seen = new Set()
    ...
    if (allObjects.length > 0) {
      this._debug(`Writing ${allObjects.length} objects and ${Object.keys(allRefs).length} refs`)
      await toDisk({
