...
class Gip extends ReadyResource {
  ...
  async addPushRefs(refs) {
    ...
    // A leading + marks a forced update. The store overwrites refs
    // unconditionally, so honouring it is just stripping the marker —
    // leaving it in sends '+refs/…' to rev-parse, which echoes the bad
    // name to stdout and poisons the delta walk.
