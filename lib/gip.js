...
class Gip extends ReadyResource {
  ...
  async fetch() {
    ...
    // Collect all unique objects across all pending fetches.
    // Branches and tags share most objects — dedup to avoid redundant writes.
  ...
  async push() {
    ...
    // Each ref's record needs its full reachable oid list, but only the oids
    // — cheap graph walks, no content.
    const allOidsByRef = new Map()
    for (const ref of toPush) {
      allOidsByRef.set(ref, await this._revListOids(ref.oid))
    }
    ...
    for (const ref of toPush) {
      ...
      try {
        ...
        // anything else from its store; allOids carries the ref's full
        // reachable list for its record.
        await this._remoteDb.push(branchName, pushedRef.oid, objects, {
          allOids: [...allOidsByRef.get(ref)]
        })
