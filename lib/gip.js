...
class Gip extends ReadyResource {
  ...
  async _getRefData(ref) {
    ...
    try {
      ...
      // Prepend the ref OID so annotated tag objects are included — git rev-list
      // dereferences tags to commits and does not emit the tag object itself.
      // sort -u deduplicates in case the OID is already in the rev-list output
      // (lightweight tags, commits).
      const objectsResult = await sh
        .pipe(`{ echo '${ref.oid}'; git rev-list --objects ${formatRef(ref)}; }`)
        .pipe("cut -d' ' -f1")
        .pipe('sort -u')
